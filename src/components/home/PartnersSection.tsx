  ...
  return (
    <section className="py-20 bg-white overflow-hidden w-full">
      ...
      <div className="relative flex overflow-x-hidden w-full">
        <div className="animate-marquee whitespace-nowrap flex items-center justify-center gap-16 px-8">
          {[...logos, ...logos].map((logo, index) => (
            <div key={index} className="inline-block">
              <img
                src={logo.src}
                alt={logo.alt}
                className={`${logo.className} w-auto object-contain opacity-80 hover:opacity-100 transition-opacity duration-300 filter grayscale hover:grayscale-0`}
              />
            </div>
          ))}
        </div>
        <div className="absolute top-0 animate-marquee2 whitespace-nowrap flex items-center justify-center gap-16 px-8">
          {[...logos, ...logos].map((logo, index) => (
            <div key={index} className="inline-block">
              <img
                src={logo.src}
